...
const users = [
    ...
    {
        name: "Ala",
        message: "Dzień dobry, ja... ⋅ 12:30",
        image: "../images/client2.jpg"
    },
    {
        name: "Kuba",
        message: "W takim razie widzi... ⋅ 12:30",
...
const shelters = [
    {
        name: "Kasia z Krakowskie Schronisko dla Bezdomnych Zwierząt",
        message: "Jasne! ⋅ 12:30",
        image: "../images/shelter1.jpg"
    },
    {
        name: "Asia z Schronisko dla zwierząt Psie Pole",
        message: "Ty: Dzień dobry, ja .... ⋅ 14:00",
        image: "../images/shelter2.jpg"
    },
    {
        name: "Basia z Schronisko dla Bezdomnych Zwierząt Nowy Targ",
        message: "Ty: Dziękuję! ⋅ 15:00",
        image: "../images/shelter3.jpg"
    },
    {
        name: "Pszczoła z Dom dla Bezdomnych Zwierząt Kryspinów",
        message: "Ty: Dziękuję! ⋅ 15:00",
        image: "../images/shelter4.jpg"
    },
];

// Funkcja, która tworzy elementy i dodaje je do kontenera
function createListItems() {
    ...
    if (profiletype === 'user') {
        displayContent = shelters;
    } else if (profiletype === 'shelter') {
        displayContent = users;
    }

    // Pętla, która wygeneruje elementy
    displayContent.forEach(shelter => {
        // Tworzenie nowego elementu (np. div)
        const listItem = document.createElement('div');
        listItem.className = 'availableChat'; // Dodanie klasy dla stylizacji

        // Tworzymy obrazek
        const profilePic = document.createElement('img');
        profilePic.src = shelter.image;
        profilePic.className = 'chat-profile-pic'; // Dodajemy klasę dla stylizacji

        // Tworzymy kontener na tekst
        const chatText = document.createElement('div');
        chatText.className = 'chat-text';

        // Tworzymy nazwę schroniska
        const shelterName = document.createElement('h1');
        shelterName.textContent = shelter.name; // Przypisujemy nazwę schroniska

        // Tworzymy wiadomość
        const shelterMessage = document.createElement('p');
        shelterMessage.textContent = shelter.message; // Przypisujemy wiadomość

        // Dodajemy elementy tekstowe do kontenera na tekst
        chatText.appendChild(shelterName);
        chatText.appendChild(shelterMessage);

        // Dodajemy obrazek i kontener tekstowy do głównego kontenera
        listItem.appendChild(profilePic);
        listItem.appendChild(chatText);

        // Dodajemy cały element do listy
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (chatList) {
        chatList.addEventListener('click', function (event) {
            ...
            // Sprawdź, czy kliknięty element lub jego rodzic ma klasę "availableChat"
            const targetElement = event.target.closest('.availableChat');
            if (targetElement) {
